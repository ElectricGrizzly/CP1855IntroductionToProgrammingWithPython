...
def drop_item(items: list[str]) -> None:
    ...
    try:
        ...
        if not _is_inventory_location(inventory_location, items):
            ...
            print(f'{item.strip()} was dropped')
...
def give_starter_items() -> list[str]:
    ...
    try:
        return _get_items_from_file(STORED_ITEMS_FILE)
...
def confirm_all_items_file():
    """Confirm that the wizard items file is present."""
    try:
        return _get_items_from_file(ALL_ITEMS_FILE)
    except FileNotFoundError:
...
def _get_items_from_file(file: str) -> list[str]:
    """Private function to read the items from a text file."""
